...
void App::UpdateEmitter()
{
	...
	for (int e = 0; e < NumEmit; e++)
	{
		...
		{	int eX = em.size, eY = (em.size2 == 0) ? eX : em.size2, size = eX*eY;
			...
			float3 ev = make_float3(0,0, em.vel*sc.params.timeStep/* (0.003f/p->timeStep)*/);//
			float4 vel4 ; mulTr(&ev.x, &vel4.x, modelView);
			...
			for (int x=0; x < eX; x++, i++)
			{
				...
				pos[i] += em.posLag;
				vel[i] = make_float4(ev.x,ev.y,ev.z,0.0f);
			}
			
  			psys->setArray(0, pos, emitId, size);  // set
			psys->setArray(1, vel, emitId, size);

			/* inc Id */  uint num = sc.params.numParticles-size;  // -size?
			emitId += size; emitId = emitId % num;
		}
	}
