...
void Scene::InitDefault()	//  Init
{
	...
	p.h = p.particleR*2.5;	 p.h2 = p.h*p.h;
	spacing = 2 /* p.particleR*/;

	p.restDensity = 1000;	p.minDens = 1.0; /* 0.9  p.restDensity*/
	p.stiffness = 3.0;  p.viscosity = 1.0;
	
	p.Poly6Kern = 315.0f / (64.0f * PI * pow(p.h, 9));
	p.SpikyKern = 0.5 * 45.0f / (PI * pow(p.h, 6));
	...
	p.bndStiff = 30000;	  p.bndDamp = 120;/*X Axis bnd*/	p.bndDampC = 60; /*Y Axis bnd*/
